...
def creer_soumission(payload: SoumissionCreate, db: Session = Depends(get_db)):
    entreprise = db.query(EntrepriseProfile).filter(EntrepriseProfile.id == payload.entreprise_id).first()
    if not entreprise:
        raise HTTPException(status_code=404, detail="Entreprise non trouvée")

    ao = None
    if payload.appel_offres_id is not None:
    ...
    elif not payload.objet_externe:
        raise HTTPException(
            status_code=400,
            detail="Fournissez soit appel_offres_id, soit objet_externe pour un appel d'offres trouvé "
                   "par vous-même.",
        )
...
def generer_documents_soumission(payload: GenererDocumentsRequest, db: Session = Depends(get_db)):
    ...
    ao = None
    if soumission.appel_offres_id is not None:
...
def rediger_dao_ia(soumission_id: int, db: Session = Depends(get_db)):
    ...
    profile = db.query(EntrepriseProfile).filter(EntrepriseProfile.id == soumission.entreprise_id).first()
    if not profile:
        raise HTTPException(status_code=404, detail="Entreprise liée à la soumission non trouvée")
